...
public class AttackGoalPatcher {
  ...
  public static void addGoal(Mob mob, String id, Sound sound, double range, boolean aggressive,
      boolean flying) {
    switch (id) {
      case "strife:generic_attack_goal" -> {
        AttackEntityGoal goalAttack = new AttackEntityGoal(mob, sound, range, aggressive, flying);
        if (!Bukkit.getMobGoals().hasGoal(mob, goalAttack.getKey())) {
          Bukkit.getMobGoals().addGoal(mob, 3, goalAttack);
        }
      }
      case "strife:flee_goal" -> {
        AttackEntityGoal goalAttack = new AttackEntityGoal(mob, sound, range, aggressive, flying);
        if (!Bukkit.getMobGoals().hasGoal(mob, goalAttack.getKey())) {
          Bukkit.getMobGoals().addGoal(mob, 3, goalAttack);
        }
